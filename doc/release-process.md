...
* Update manpages, see [gen-manpages.sh](https://github.com/farmcoin-project/farmcoin/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/farmcoin-project/farmcoin.git
...
### Build and sign Farmcoin Core for Linux, Windows, and OS X:

    pushd ./gitian-builder
    ./bin/gbuild --num-make 2 --memory 3000 --commit farmcoin=v${VERSION} ../farmcoin/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs.ltc/ ../farmcoin/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gsign --signer $SIGNER --release ${VERSION}-win-unsigned --destination ../gitian.sigs.ltc/ ../farmcoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gsign --signer $SIGNER --release ${VERSION}-osx-unsigned --destination ../gitian.sigs.ltc/ ../farmcoin/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. OS X unsigned installer and dist tarball (`farmcoin-${VERSION}-osx-unsigned.dmg`, `farmcoin-${VERSION}-osx64.tar.gz`)
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys.
    ...
    gpg --import farmcoin/contrib/gitian-keys/*.pgp
    gpg --refresh-keys
...
Codesigner only: Sign the osx binary:
    ...
    transfer farmcoin-osx-unsigned.tar.gz to osx for signing
...
- Detached signatures will then be committed to the [farmcoin-detached-sigs](https://github.com/farmcoin-project/farmcoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed OS X binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../farmcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs.ltc/ ../farmcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
...
Create (and optionally verify) the signed Windows binaries:
    ...
    ./bin/gsign --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs.ltc/ ../farmcoin/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-win-signed ../farmcoin/contrib/gitian-descriptors/gitian-win-signer.yml
...
- Announce the release:
  ...
  - farmcoin-dev and farmcoin-dev mailing list

  - blog.farmcoin.org blog post

  - Update title of #farmcoin and #farmcoin-dev on Freenode IRC

  - Optionally twitter, reddit /r/Farmcoin, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/farmcoin-project/farmcoin/releases/new) with a link to the archived release notes.
