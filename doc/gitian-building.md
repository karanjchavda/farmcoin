...
*Setup instructions for a Gitian build of Farmcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Farmcoin
...
- [Building Farmcoin Core](#building-farmcoin-core)
...
Clone the git repositories for farmcoin and Gitian.
...
in the farmcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Farmcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the farmcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/farmcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/farmcoin-project/farmcoin
...
For example:
```bash
URL=https://github.com/thrasher-/farmcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit farmcoin=${COMMIT} --url farmcoin=${URL} ../farmcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit farmcoin=${COMMIT} --url farmcoin=${URL} ../farmcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit farmcoin=${COMMIT} --url farmcoin=${URL} ../farmcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the farmcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../farmcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/farmcoin
SIGPATH=/some/root/path/farmcoin-detached-sigs

./bin/gbuild --url farmcoin=${BTCPATH},signature=${SIGPATH} ../farmcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/farmcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/farmcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/farmcoin-osx-build.assert
```
...
[farmcoin-project/gitian.sigs.ltc](https://github.com/farmcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
